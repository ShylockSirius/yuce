...
print(all_features.shape)  # (1315, 11)

# 转成NDArray
n_train = train_data.shape[0]
train_features = torch.tensor(all_features[:n_train].values, dtype=torch.float)
test_features = torch.tensor(all_features[n_train:-1].values, dtype=torch.float)
train_labels = torch.tensor(train_data.等径长度.values, dtype=torch.int).view(-1, 1)
# print(train_labels[:])
...
def log_rmse(net, features, labels):
    with torch.no_grad():
        ...
        clipped_preds = torch.max(net(features), torch.tensor(1.0))+1e-5
        rmse = torch.sqrt(2 * loss(clipped_preds.log(), (labels+1e-5).log()).mean())
    return rmse.item()
...
# 模型选择
k, num_epochs, lr, weight_decay, batch_size = 5, 500, 5, 0, 256
