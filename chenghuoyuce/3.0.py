...
if __name__ == '__main__':
    ...
    for i in range(len(dataSet)-1):
        ...
        for j in range(len(dataSet[0])-2):
            # print(dataSet[i][j])
            a.append(dataSet[i+1][j+2])
        dataSet1.append(a)
    # print(dataSet1)
    column_to_float(dataSet1)  # dataSet
    n_folds = 5
    max_depth = 15
    min_size = 1
    ratio = 1.0
    ...
    n_features = 11
    n_trees = 10
    folds = spiltDataSet(dataSet1, n_folds)  # 先是切割数据集
    scores = []
    """
    for fold in folds:
        train_set = folds[
    ...
    """
    testSet = loadCSV('./test1.csv')
    testSet1 = []
    # print(dataSet)
    for i in range(len(testSet) - 1):
        a = []
        for j in range(len(testSet[0]) - 2):
            # print(dataSet[i][j])
            a.append(testSet[i + 1][j + 2])
        testSet1.append(a)
    id = []
    for i in range(len(testSet) - 1):
        id.append(testSet[i+1][0])
    # print(dataSet1)
    column_to_float(testSet1)  # dataSet
    predict_values = random_forest(dataSet1, testSet1, ratio, n_features, max_depth, min_size, n_trees)
    actual = [row[-1] for row in testSet1]
    accur = accuracy(predict_values, actual)
    print('Trees is %d' % n_trees)
    print('scores:%s' % accur)
    df = {"Id": id, "等径长度": predict_values}
