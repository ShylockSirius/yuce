...
def get_data_ch7(): # 不同飞机机翼噪音数据集，本函数已保存在d2lzh_pytorch包中⽅便以后使⽤
    ...
    train_data = pd.read_csv('train1.csv', encoding='ANSI')
    test_data = pd.read_csv('test1.csv', encoding='ANSI')
    ...
    train_labels = torch.tensor(train_data.等径长度.values, dtype=torch.float).view(-1, 1)
...
def train_pytorch_ch7(optimizer_fn, optimizer_hyperparams, features, labels, test_data, test_features, batch_size=128, num_epochs=500):
    # 初始化模型
    ...
    test_data['等径长度'] = pd.Series(preds.reshape(1, -1)[0])
    submission = pd.concat([test_data['Id'], test_data['等径长度']], axis=1)
